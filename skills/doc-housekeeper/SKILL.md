...
Identify factual "known states" of **present** assets in non-registry files and migrate them.
...
- **Action**:
    ...
    2. Move data related to **currently present** hardware to `docs/registry/`.
    3. **Exclude** all planned, proposed, or aspirational hardware (keep these in `identity/` or `workflow/`).
    4. Add a link in the registry back to the original source.
